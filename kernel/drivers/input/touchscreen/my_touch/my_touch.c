            ...
            const struct i2c_device_id *id)
{
    ...
    if(!ts) {
        dev_err(&client->dev, "Failed to allocate GFP_KERNEL memory for my_touch_dev\n");
        return -ENOMEM;
    }

    ts->client = client;
    i2c_set_clientdata(client, ts);             // 将my_touch_dev结构体指针与i2c_client关联

    // 从设备树中读取触摸屏的最大X和Y值
    if(of_property_read_u32(np, "max-x", &ts->abs_x_max)) {
        dev_err(&client->dev, "Failed to get max-x from device tree\n");
        return -EINVAL;
    }
    MY_DEBUG("abs_x_max=%d", ts->abs_x_max);

    if(of_property_read_u32(np, "max-y", &ts->abs_y_max)) {
        dev_err(&client->dev, "Failed to get max-y from device tree\n");
        return -EINVAL;
    }
    MY_DEBUG("abs_y_max=%d", ts->abs_y_max);
    ...
    ret = devm_gpio_request_one(&client->dev,ts->irq_pin, GPIOF_IN, "my touch");
    if (ret < 0){
        return ret;
    }
